import { PrismaClient, StoreCategory, WalletTransactionType } from '@prisma/client';
...
async function seedDemoUser() {
  ...
  await prisma.wallet.upsert({
    ...
    update: { currency: 'LC' },
    create: { userId: user.id, currency: 'LC' },
  });
  ...
  for (const transaction of seedTransactions) {
    await prisma.walletTransaction.upsert({
      where: { auditId: transaction.auditId },
      update: transaction,
      create: { ...transaction, userId: user.id },
    });
  }
  ...
  for (const game of games) {
    await prisma.rewardCap.upsert({
      where: { userId_gameSlug: { userId: user.id, gameSlug: game.slug } },
      update: {
...
async function main() {
  ...
  await seedStore();
  await seedDemoUser();
  console.log('Seeded persistent LezGamez MVP data.');
}
