...
export const games: Game[] = rawGames.map(([slug,title,shortDescription,category,live],i)=>({id:`game_${i+1}`,slug,title,status: live?'live': i<12?'beta':'coming_soon',shortDescription,fullDescription:`${title} is a fast mobile-first WagonBug Arcade experience built for 30 second to 3 minute runs, instant retry, rankings, internal Bug Coins and cosmetic rewards only.`,category,tags:[category,'no-download','mobile','arcade'],thumbnail:`/art/${slug}.webp`,ogImage:`/og/${slug}.png`,isPlayable:Boolean(live),gameUrl:`/games-builds/${slug}/index.html`,gameType:'iframe',supportedDevices:['mobile','desktop'],controls:['touch','mouse','keyboard'],rewardRules:{baseCoins:15+i,dailyCap:250},adRules:{interstitialEveryDeaths:3,banner:true},storeCompatibility:['skins','trails','profile_badges'],releaseDate:'2026-06-28',seoTitle:`Play ${title} Online — WagonBug Arcade`,seoDescription:shortDescription,faq:[{q:`Is ${title} free?`,a:'Yes. WagonBug Arcade is ad-supported and Bug Coins are internal credits only.'},{q:'Can I withdraw Bug Coins?',a:'No. Bug Coins cannot be withdrawn, sold, transferred or exchanged for real money.'}],relatedGames:[]}));
